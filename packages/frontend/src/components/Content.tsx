...
export const Content: React.VFC = () => {
  const initArray = [<Card key="0" title="" date="" weekday="" labels={[]}>Not loaded.</Card>]
  const [cards, setCards] = useState(initArray);
    ...
    useEffect(() => {
      (async function () {
        ...
        for (let i = 0; i < cardJson.length; i++) {
          const card = cardJson[i];
          console.log(card)
          cardsArr.push(<Card title="nice title" date="25" weekday="MON" labels={["important","cool"]}>What am I going to do?</Card>);
        }
